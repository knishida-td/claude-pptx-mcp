...
■ KeyMsg背景: 全スライドにFFF5F0背景矩形(0.5,4.837,9.0,0.4)を配置。削除禁止
...
### python-pptxでネイティブ図解を追加する際のルール
pptx_generate後にpython-pptxで図形（円・矩形・矢印等）を追加する場合:

**テンプレートシェイプの保護（絶対に削除しない）**:
- Text 0（タイトル）、Shape 1/2（アンダーライン）、Text 7（KeyMsgテキスト）、Text 8（ページ番号）
- KeyMsg背景: fill=FFF5F0の矩形 at (0.5, 4.837, 9.0, 0.4)。全スライド必須

**bigtext（図解用）スライドで消してよいシェイプ**:
- Text 3（空のbigtextヘッダー 9.0x1.0）、Shape 4（中央の区切り線 5.0x0.015）、Text 5（空のbigtextサブテキスト 9.0x0.6）

**フォントサイズ統一（図解スライド）**:
- ヒーロー（大型シェイプ内ラベル）: 18pt bold
- 見出し（カード・項目タイトル）: 13pt bold
- 本文（説明・メッセージ）: 11pt
- キャプション（注釈・小ラベル）: 9pt

**不要改行チェック（必須）**:
全テキストについて、推定テキスト幅がボックス幅（内部マージン14.4pt差引）を超えないか検証する。
全角文字≈font_size×1.05pt、半角英数≈font_size×0.6pt。超過する場合はテキスト短縮またはフォント縮小。

**よくあるバグ**:
- Inches()の二重適用: \`label_w = Inches(0.7)\` の後に \`Inches(label_w)\` とすると640,080インチになる
- 背面配置: \`sp_tree.insert(2, elem)\` でnvGrpSpPr/grpSpPrの後に挿入
- 左右バランス: 暗い要素は視覚的に重いため、数学的中央より視覚的中央を優先

**ヘルパー・パターン**: scripts/diagram_helpers.py にDiagramKitクラスと5つの図解パターン関数あり。
詳細はリソース pptx://diagrams を参照

### 画像挿入のルール
**検索**: scripts/image_search.py でぱくたそのタグページから検索・DL。
\`search_and_download(query, save_path, index=N)\` でN番目の候補を取得。

**挿入前の確認ループ（必須）**:
1. DLした画像を表示して内容を確認
2. NG（ネタ写真・文脈に合わない）なら index を変えて次の候補をDL
3. OKが出てから初めてPPTXに挿入

**アスペクト比の維持（厳守）**:
- \`add_picture(path, x, y, width=Inches(w))\` — **widthのみ指定**。heightは自動計算
- width/height両方指定は**禁止**（引き伸ばしが発生する）
- \`fit_image_size(path, max_w, max_h)\` で事前にサイズ計算

**レイアウト**: 写真を後付けしない。写真を入れる場合はスライドを完全に作り直す。
- 配置順: 見出し → 箇条書き → **写真（下部）**。写真は視覚補強であって主役ではない
- 3カラム+写真: 各カラムの下に写真。写真とテキストの間に0.1"以上のギャップ

**画像の取得方法（2パターン）**:
1. **日本人ビジネス写真**: scripts/image_search.py の \`search_and_download(tag, path)\` でぱくたそから取得
2. **商品・商材画像**: Claude CodeのWebSearchツールで画像URLを検索 → \`download_image(url, path)\` でDL
   - WebSearchで「商品名 公式」「ブランド名 商品画像」等で検索
   - 結果のURLからWebFetchで画像直URLを特定
   - scripts/image_search.py の \`download_image()\` と \`fit_image_size()\` でDL+サイズ計算
...
const RESOURCE_FILES: Array<{
  ...
  {
    uri: "pptx://diagrams",
    name: "Native Diagram Patterns",
    description: "python-pptxネイティブ図解パターン（DiagramKit、横フロー、カードグリッド、2カラム比較、チップ一覧、人物カード）",
    file: "diagrams.md",
  },
];
