...
## クレジット軽量化ルール（エージェント使用時）

資料作成は完全自動で実行するが、エージェント数とトークン消費を最小化すること。

### リサーチ
- **1エージェントに統合**: 一次リサーチ+ファクトチェック（公式サイト確認）+商品画像DLを1本のエージェントにまとめる。分割しない

### QA
- **1エージェントで全スライド検査（sonnet指定）**: 24枚を分割せず1本のsonnetエージェントに渡す。opusは不要、haikuは折り返し検出が弱いのでsonnet

### ドキュメント読み込み
- **Skillドキュメントの全文読みを省略**: slidekit.md/pptxgenjs.md/japanese-market-rules.mdはメモリとINSTRUCTIONSで把握済み。必要な場合のみリソース参照
- **リファレンススクリプトは先頭180行のみ**: ヘルパー関数の構造だけ確認すれば十分

### パイプライン
- **validate→node→pdf変換→画像化を1つのBashチェーンで実行**: ラウンドトリップを減らす
`;
